...
| `bigint` | `int` (ou `string` si hors plage PHP) | BIGINT | Très grand nombre |
...
>
> **Note bigint (DBAL 4)** : depuis Doctrine DBAL 4, un `bigint` est converti en `int` PHP quand la valeur reste dans la plage des entiers PHP (cas normal sur PHP 64 bits). Il redevient une `string` seulement hors plage. Sur d'anciennes docs DBAL 2/3, le type PHP était toujours `string`.
>
