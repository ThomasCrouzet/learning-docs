...
```mermaid
flowchart LR
    data[Données soumises] --> assert[Contraintes Assert<br>sur l'entité]
    assert --> validator[Validator<br>Vérifie les règles]
    validator -->|Valide| ok[Données acceptées]
    validator -->|Invalide| errors[Erreurs affichées]
```
