...
> **Sécurité** : l'exemple ci-dessous illustre uniquement le hashage. En production, utilise un vrai `FormType` Symfony (protection CSRF incluse), valide email et mot de passe, et ne stocke jamais le mot de passe en clair. Un formulaire HTML brut sans token CSRF est vulnérable aux envois forgés.
