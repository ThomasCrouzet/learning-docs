...
| Failles de sécurité | Utiliser des fonctions sécurisées (`fgets` au lieu de `gets`, `strncpy` au lieu de `strcpy`) |
...
```bash
gcc -Wall -Wextra -Werror tableaux.c -o tableaux
./tableaux
```
...
```bash
gcc -Wall -Wextra -Werror chaines.c -o chaines
./chaines
```
...
```bash
gcc -Wall -Wextra -Werror string_fonctions.c -o string_fonctions
./string_fonctions
```
...
```bash
gcc -Wall -Wextra -Werror securite.c -o securite
