...
total_fiches: 10
cursus: "Node.js"
id: "fundamentals.nodejs.introduction-nodejs"
...
```bash
# Installe Node.js 22 LTS (version cible de ce cursus)
# Note : la formule `node` sans numéro peut installer la branche Current (ex. 26.x)
brew install node@22
# node@22 est keg-only : lie-le pour que `node` et `npm` soient dans le PATH
...
```bash
# Ajouter le dépôt NodeSource (branche 22.x LTS)
...
2. Télécharge la version LTS **22.x** (celle utilisée dans ce cursus ; Node.js 24 LTS est aussi acceptable)
...
> **Note** : Node.js 20 LTS est en fin de support (EOL depuis avril 2026). N'installe pas `node@20` pour ce cursus.
