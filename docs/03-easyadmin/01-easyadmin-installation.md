...
> **Note de version** : Ce cursus couvre **EasyAdmin 4.x** (API stable pour les exercices). Depuis 2025-2026, **EasyAdmin 5.x est la version stable** (maintenance active) et la branche 4.x ne reçoit plus que des correctifs. La commande ci-dessous épingle la version 4 pour rester aligné avec les fiches. Si tu vois `Using version ^5.x` dans la sortie, consulte le `UPGRADE.md` du bundle avant de continuer.
