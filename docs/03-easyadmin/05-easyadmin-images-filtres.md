...
Le second import (`Crud`) sert ici pour `Crud::PAGE_NEW`, mais il servira aussi à la méthode `configureCrud` de la Partie 5. Un seul `use` en tête de fichier suffit pour tout le contrôleur.
...
**⚠️ Important** : Le dossier `public/uploads/images/products` doit pouvoir être créé par PHP. Symfony le fait automatiquement, mais assure-toi que les permissions sont bonnes.
...
| `php bin/console debug:config easy_admin` | Afficher la configuration EasyAdmin active |
