...
```bash
# Installe Ansible dans l'environnement virtuel
pip install ansible
```
...
| --- | --- |
| `sudo apt install ansible` | `pip install ansible` |
| Version souvent ancienne (dépend de la distribution) | Dernière version stable |
| Installé globalement sur le système | Isolé dans l'environnement virtuel |
| Mise à jour liée à la distribution | Mise à jour indépendante avec `pip install --upgrade ansible` |

Recommandation : Utilise **toujours** pip pour installer Ansible. La version disponible via apt est souvent en retard de plusieurs versions majeures.
...
| `pip install ansible` | Installe Ansible dans l'environnement virtuel actif |
...
**Solution** : Installe toujours Ansible avec `pip install ansible` dans un environnement virtuel Python, comme décrit dans les étapes 2 et 3.
