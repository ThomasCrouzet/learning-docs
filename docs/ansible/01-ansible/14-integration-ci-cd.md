...
sequenceDiagram
    ...
    participant S as Serveurs

    G->>CI: Push / Merge
    CI->>A: ansible-playbook deploy.yml
    A->>S: Déploiement via SSH
    S-->>CI: Résultat
```
...
**Solution** : Ajoute toujours `pip install ansible` dans la section `before_script` de chaque job qui utilise Ansible.
