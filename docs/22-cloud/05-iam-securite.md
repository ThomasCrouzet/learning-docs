...
# Creer des cles d'acces pour le CLI
...
Note bien la clé secrete : elle n'est affichee qu'une seule fois.
...
**Solution** : Créé un utilisateur IAM avec les permissions `AdministratorAccess` pour les taches quotidiennes. Active le MFA sur le compte root et ne l'utilise que pour les opérations qui l'exigent (changement du plan de support, fermeture du compte).
