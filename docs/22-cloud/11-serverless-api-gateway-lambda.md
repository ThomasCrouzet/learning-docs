...
- Le serverless n'est pas adapté à tous les cas d'usage. Les traitements longs (plus de 15 minutes pour Lambda), les connexions persistantes (WebSockets, base de données), ou les applications nécessitant un état local ne sont pas adaptés au serverless.
...
| Taille du package de déploiement | 50 Mo (zip), 250 Mo décompressé |
