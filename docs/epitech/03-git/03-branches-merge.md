...
### Étape 6 : Revenir à master et voir la différence
...
```text
a1b2c3d (HEAD -> master, feature/header) Ajout du titre dans le header
...
# Modifier la même partie sur master
...
⚠️ **Problème** : Tu fais des commits sur `master` au lieu de ta branche.
...
```bash
# Pour merger feature dans master :
# 1. Aller sur master
git checkout master
# 2. Merger feature
git merge feature
...
3. Crée une branche `feature/contact` (depuis master) et ajoute `contact.html`
4. Merge `feature/articles` dans `master`
5. Merge `feature/contact` dans `master`
...
# 3. Branche contact (depuis master)
git checkout master
...
```text
*   a1b2c3d (HEAD -> master) Merge feature/contact
|\
