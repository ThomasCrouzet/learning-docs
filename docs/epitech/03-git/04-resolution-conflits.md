...
```text
[master (root-commit) abc1234] Ajout de la configuration initiale
...
```text
master :             port = 5000, debug = false, timeout = 30
...
- `<<<<<<< HEAD` : début de la version actuelle (master, qui contient déjà le merge de dev-config)
...
```text
*   f1e2d3c (HEAD -> master) Merge feature/prod-config : config combinée dev+prod
|\
...
# Revenir à master et modifier aussi
git checkout master
...
```bash
# Tu es sur master et tu fais git merge feature
# --ours   = master (ta branche)
...
5. Reviens sur `master` et modifie les mêmes lignes différemment
...
```bash
# 5. Revenir à master et modifier différemment
git checkout master
...
*   f1e2d3c (HEAD -> master) Merge feature/production : serveur combiné, app en version prod
|\
