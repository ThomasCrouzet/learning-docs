...
```text
ghi9012 (HEAD -> master) Ajout thème sombre, version 1.2
...
Tu vas créer une branche, faire des commits dessus, puis la rebaser sur `master` après que `master` a avancé.

```bash
# Créer une branche feature depuis master
...
Maintenant, simule un avancement de `master` :

```bash
# Revenir sur master et ajouter un commit
git checkout master
...
Les deux branches ont divergé depuis `ghi9012`. Maintenant, rebase `feature/api` sur `master` :
...
* jkl3456 (master) Ajout configuration base de données
* ghi9012 Ajout thème sombre, version 1.2
* def5678 Ajout fonctionnalité login, version 1.1
* abc1234 Commit initial : app.txt version 1.0
```

L'historique est linéaire. Les commits de `feature/api` sont maintenant après le commit de `master`.

```bash
# Finaliser : merger feature/api dans master (fast-forward)
git checkout master
...
```text
Saved working directory and index state On master: Dashboard en cours de développement
On branch master
...
```text
stash@{0}: On master: Dashboard en cours de développement
On branch master
Changes not staged for commit:
        modified:   app.txt
...
Tu veux uniquement le fix du port sur `master`, pas le commit redis non terminé.
...
```bash
# Revenir sur master et cherry-pick uniquement le commit de fix
git checkout master
...
```text
[master efg1234] Fix : correction du port PostgreSQL (5433)
 1 file changed, 1 insertion(+), 1 deletion(-)
```

Seul le fix du port est appliqué sur `master`. Le commit redis est resté sur `bugfix/correction-port`.
...
```text
efg1234 (HEAD -> master, tag: v1.1, tag: v1.0) Fix : correction du port PostgreSQL (5433)
...
```text
stu5678 (HEAD -> master) Ajout endpoint POST /products
vwx9012 Ajout endpoint GET /users
...
```text
stu5678 (HEAD -> master) HEAD@{0}: reset: moving to HEAD~1
...
```text
[master klm9012] Revert "Changement vers MySQL (erreur)"
...
klm9012 (HEAD -> master) Revert "Changement vers MySQL (erreur)"
...
4. Reviens sur `master` et fais 1 commit (ajout config)
5. Rebase `feature/auth` sur `master` et fais un fast-forward merge
...
8. Reviens sur `master` et cherry-pick uniquement le commit "ajout header"
9. Récupère le stash
...
# 4. Revenir sur master et ajouter un commit
git checkout master
cat > config.txt << 'EOF'
# Config
env = development
...
# 5. Rebase feature/auth sur master et fast-forward
...
# 8. Cherry-pick uniquement le header sur master
