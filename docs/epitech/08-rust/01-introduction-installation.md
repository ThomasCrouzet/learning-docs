...
3. **Sécurité compromise** : 70% des vulnérabilités de sécurité dans les logiciels viennent d'erreurs de mémoire.

**Comment Rust résout ces problèmes** :

| Problème | Solution apportée par Rust |
| --- | --- |
| Erreurs de mémoire | Le compilateur vérifie la mémoire AVANT l'exécution |
| Bugs difficiles à trouver | Si le code compile, il n'a pas d'erreur de mémoire |
| Sécurité compromise | Impossible d'avoir ces vulnérabilités par accident |
...
- Rust n'est pas difficile à apprendre parce qu'il est mal conçu. Il est strict parce qu'il t'empêche de faire des erreurs.
