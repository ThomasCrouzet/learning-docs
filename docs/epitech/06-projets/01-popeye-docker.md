...
db:
  image: postgres:16
  healthcheck:
    test: ["CMD-SHELL", "pg_isready -U popeye"]
    interval: 5s
    timeout: 5s
    retries: 5
...
**Énoncé** : Étendre le projet Popeye en ajoutant un service Nginx en reverse proxy devant l'application poll. Le Nginx doit écouter sur le port 80 et rediriger le trafic vers le service poll. Ajouter un healthcheck Docker sur le service Nginx.
