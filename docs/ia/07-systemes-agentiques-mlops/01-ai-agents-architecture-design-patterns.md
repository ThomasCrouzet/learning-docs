...
**Définition** : Un agent IA est un système autonome composé d'un LLM (Large Language Model) couplé à des outils et à une boucle de raisonnement. L'agent reçoit un objectif, décide des actions à entreprendre, exécute ces actions via des outils, observe les résultats et itère jusqu'à atteindre l'objectif.
...
2. **Pas de boucle de rétroaction** : un LLM répond en une seule passe. Il ne peut pas vérifier si sa réponse est correcte ni corriger ses erreurs.
3. **Tâches complexes impossibles** : les tâches qui nécessitent plusieurs étapes (rechercher, analyser, rédiger) ne peuvent pas être accomplies en un seul appel.
...
- Un agent IA n'est pas un chatbot. Un chatbot répond à des messages. Un agent prend des décisions et exécute des actions de manière autonome.
- Un agent IA n'est pas un script automatisé. Un script suit un chemin prédéfini. Un agent décide dynamiquement de ses actions en fonction du contexte.
