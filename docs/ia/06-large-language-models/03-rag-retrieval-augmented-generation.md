...
| Hallucinations | Le LLM s'appuie sur des documents réels pour générer sa réponse |
| Pas de sources vérifiables | Chaque réponse peut citer les documents sources utilisés |
| Pas de données privées | Les documents internes sont indexés dans le vector store |

**Analogie concrète** : Le RAG fonctionne comme un étudiant qui a le droit de consulter ses notes pendant un examen. Sans ses notes (sans RAG), il répond de mémoire et risque de se tromper. Avec ses notes (avec RAG), il retrouve les informations exactes dans ses fiches avant de formuler sa réponse.
...
- Le RAG n'est pas une simple copie de documents. Le LLM synthétise et reformule l'information trouvée pour répondre précisément à la question posée.
