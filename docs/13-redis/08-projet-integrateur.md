...
<p><a href="../../diagrams/13-redis-08-projet-integrateur-1.html">Architecture de l&#x27;application (HTML + SVG)</a></p>
<iframe src="../../diagrams/13-redis-08-projet-integrateur-1.html" title="Architecture de l&#x27;application" style="width:100%;min-height:516px;border:0;background:transparent"></iframe>
</div>
...
services:
  ...
  redis:
    ...
    volumes:
      - redis_data:/data
    # AOF pour la persistance ; noeviction requis pour redis_tag_aware
    # (et pour ne pas supprimer sessions / messages Messenger)
...
| `--maxmemory-policy noeviction` | Refuse les écritures si la mémoire est pleine (ne supprime aucune clé) |

⚠️ **Pourquoi `noeviction` ici** : Symfony documente que `cache.adapter.redis_tag_aware` exige une politique `noeviction` ou `volatile-*` pour préserver les liens entre tags et entrées de cache. De plus, avec sessions et Messenger sur la même instance, une politique `allkeys-lru` peut supprimer des sessions actives ou des messages encore en attente.

⚠️ **Production** : sépare au minimum le cache (éviction possible avec `volatile-lru` si toutes les clés de cache ont un TTL) des sessions et des files Messenger (`noeviction`, ou une instance Redis dédiée).
