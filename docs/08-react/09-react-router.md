...
> **Note sur la version** : `npm install react-router-dom` installe la version 7.x (mode "library"). Cette fiche utilise l'API classique (`BrowserRouter`, `Routes`, `Route`, `useParams`, `useNavigate`) qui fonctionne en React Router v7 mode "library". Si tu consultes la documentation officielle `reactrouter.com`, elle met en avant le mode "framework" (basé sur Remix) qui utilise une API différente (`createBrowserRouter`). Pour ce cursus, reste sur l'API de cette fiche.
...
| Commande | Action |
| --- | --- |
| `npm install react-router-dom` | Installe React Router |
