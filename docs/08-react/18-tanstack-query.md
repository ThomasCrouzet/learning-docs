...
function ListeArticles() {
  ...
  // v5 : isPending = pas encore de data (état initial) ; isError ; data
  // (isLoading existe encore = isPending && isFetching ; la doc officielle
  // recommande isPending pour le premier chargement)
  const { data, isPending, isError, error } = useQuery({
    queryKey: ["articles"],
    queryFn: recupererArticles,
  });

  // Pendant le premier chargement (aucune donnée en cache)
  if (isPending) {
...
function DetailArticle({ id }: { id: number }) {
  ...
  if (isPending) return <p>Chargement de l'article...</p>;
...
function ListeUtilisateurs() {
  ...
  if (isPending) return <p>Chargement des utilisateurs...</p>;
