...
id: "infrastructure.cicd.github-actions-premiers-pas"
...
| Action réutilisable | `uses` | `uses: actions/checkout@v7` |
...
| Action | Rôle |
| --- | --- |
| `actions/checkout@v7` | Récupère le code du dépôt dans le runner (major courant en 2026 ; v5/v6 restent utilisables) |
| `actions/setup-node@v7` | Installe Node.js sur le runner |
| `actions/setup-java@v5` | Installe Java sur le runner |
...
- Un runner n'est pas ton ordinateur. Le code n'est pas présent sur le runner par défaut. Tu dois utiliser `actions/checkout@v7` pour le récupérer.
...
⚠️ **Problème** : Tu écris un workflow qui compile du code, mais tu oublies l'étape `actions/checkout@v7`. Le runner ne contient pas ton code. Le build échoue avec "fichier introuvable".

✅ **Solution** : Ajoute toujours `actions/checkout@v7` comme première étape si ton workflow a besoin du code du dépôt.
...
2. Contient deux jobs :
   ...
   - `check` : récupère le code avec `actions/checkout@v7`, puis affiche la liste des fichiers. Ce job doit attendre la fin du job `info`.
...
- N'oublie pas `actions/checkout@v7` avant de lister les fichiers
...
- `actions/checkout@v7` : récupère le code avant de pouvoir le lister
