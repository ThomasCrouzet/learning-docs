...
jobs:
  deploy:
    runs-on: ubuntu-latest
    # GITHUB_TOKEN n'a pas packages:write par défaut.
    # Sans ce bloc, un push vers GHCR échoue avec "permission denied".
    permissions:
      contents: read
      packages: write
    steps:
      ...
      - name: Se connecter au registry
        ...
        with:
          ...
          # Il faut packages: write (bloc permissions ci-dessus) pour pousser vers GHCR
