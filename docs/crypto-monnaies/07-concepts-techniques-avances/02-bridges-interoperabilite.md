...
```mermaid
flowchart LR
    user[Utilisateur] -->|Envoie ETH| lock[Smart contract<br>Verrouillage]
    lock -->|Preuve| bridge[Bridge<br>Vérification]
    bridge -->|Mint| weth[WETH créé<br>sur chaîne B]

    subgraph chainA[Chaîne A - Ethereum]
        lock
    end
    subgraph chainB[Chaîne B - Polygon]
        weth
    end
```
