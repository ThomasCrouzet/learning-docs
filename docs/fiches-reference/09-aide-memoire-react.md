...
// Mémoriser une valeur calculée (copie avant sort : sort mute le tableau d'origine)
...
Depuis React Router v8, le paquet s'appelle `react-router` (le paquet `react-router-dom` a été retiré).
