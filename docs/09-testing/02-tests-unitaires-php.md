...
**Définition** : PHPUnit est le framework de tests unitaires standard pour PHP. Il fournit des outils pour écrire, organiser et exécuter des tests automatisés. Cette fiche installe **PHPUnit 12** (`composer require --dev phpunit/phpunit:^12.0`), la ligne adaptée à **PHP 8.3** (version de référence du projet). L'API d'assertions et les attributs PHPUnit 10+ restent proches entre 11, 12 et 13.

**Versions et support (à vérifier sur [phpunit.de/supported-versions](https://phpunit.de/supported-versions.html))** :

| Ligne | PHP minimum | Notes (état juillet 2026) |
| ----- | ----------- | ------------------------- |
| PHPUnit 11 | >= 8.2 | Fin des correctifs de bugs depuis le **6 février 2026** ; encore rencontrée sur des projets existants |
| PHPUnit 12 | >= 8.3 | Ligne recommandée pour PHP 8.3 (correctifs jusqu'au **5 février 2027**) |
| PHPUnit 13 | >= 8.4 | Ligne stable courante si tu es sur PHP 8.4+ |

La commande `composer show phpunit/phpunit` indique la version réellement installée. Adapte les sorties d'exemple (`PHPUnit 12.x.x`) si le numéro diffère. Sur PHP 8.4+, tu peux installer `^13.0` à la place.
...
# Installe PHPUnit 12 comme dépendance de développement (PHP 8.3+)
...
...
Installing phpunit/phpunit (12.x.x)
```
...
| Commande | Action |
| --- | --- |
| `composer require --dev phpunit/phpunit:^12.0` | Installer PHPUnit 12 (PHP 8.3+) |
...
- [ ] J'ai installé PHPUnit 12 avec Composer
