...
- Utilise `np.random.seed(42)` puis `np.random.uniform(-5, 15, 31)` pour générer les températures
...
**Résultat attendu** (avec seed 42) :

```text
Temperatures de janvier (31 jours)
Moyenne     : 4.76 C
Ecart-type  : 5.40 C
Jour le plus froid : jour 30 (-3.85 C)
Jour le plus chaud : jour 26 (14.30 C)
Jours de gel (< 0) : 6
Semaine 2 (jours 8-14) : [9.74 -0.48 4.76 12.42 1.46 7.49 10.44]
```
...
# Fixer le generateur aleatoire pour des resultats reproductibles
np.random.seed(42)

# Generer 31 temperatures entre -5 et 15 degres
temperatures = np.random.uniform(-5, 15, 31)
