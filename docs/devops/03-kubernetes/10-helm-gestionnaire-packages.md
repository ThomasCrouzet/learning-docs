...
```bash
# Ajoute le dépôt de charts Bitnami (encore largement documenté)
...
⚠️ **Changements Bitnami (depuis août 2025)** : le catalogue public gratuit a été restreint. Beaucoup d'images versionnées ont été déplacées vers `docker.io/bitnamilegacy` et ne reçoivent plus de mises à jour de sécurité. Les charts Helm Bitnami peuvent encore s'installer, mais le pull d'image échoue ou tire une image non maintenue si tu ne surcharges pas le dépôt d'images.

Pour ce lab, deux options fiables :

1. **Préférer un chart local** créé avec `helm create` (étapes plus bas) pour apprendre install / upgrade / rollback sans dépendre d'un registry tiers
2. **Si tu utilises un chart Bitnami**, surcharge le dépôt d'images vers le legacy, par exemple avec un fichier values :

```yaml
# bitnami-legacy-images.yaml (lab uniquement, images non maintenues)
global:
  security:
...
En production, choisis des charts maintenus (Artifact Hub, charts officiels du projet) et des images toujours mises à jour. Ne base pas un cluster de production sur `bitnamilegacy`.
...
# Si le pull d'image échoue après les changements Bitnami 2025, ajoute :
#   -f bitnami-legacy-images.yaml
# (voir l'avertissement de l'étape 2) ou utilise un chart local (helm create)
